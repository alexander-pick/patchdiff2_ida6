...
{
	backup_free_node((char *)"$ pdiff2_matched", eng->mlist ? eng->mlist->org_num : 0);
	backup_free_node((char *)"$ pdiff2_identical", eng->ilist ? eng->ilist->org_num : 0);
	backup_free_node((char *)"$ pdiff2_unmatched", eng->ulist ? eng->ulist->org_num : 0);

	backup_free_node((char *)"$ pdiff2_eng", 1);
}
...
{
	backup_save_eng((char *)"$ pdiff2_eng", eng);

	backup_save_list((char *)"$ pdiff2_matched", eng->mlist);
	backup_save_list((char *)"$ pdiff2_identical", eng->ilist);
	backup_save_list((char *)"$ pdiff2_unmatched", eng->ulist);
}
...
int backup_load_results(deng_t ** eng, options_t * opt)
{
	...
	*eng = backup_load_eng((char *)"$ pdiff2_eng", opt, &version);
	if (!(*eng)) return 0;
	...
	if (!backup_load_list((char *)"$ pdiff2_matched", (*eng)->mlist, 1, version)) goto error;
	if (!backup_load_list((char *)"$ pdiff2_identical", (*eng)->ilist, 1, version)) goto error;
	if (!backup_load_list((char *)"$ pdiff2_unmatched", (*eng)->ulist, 0, version)) goto error;
	msg("done.\n");
